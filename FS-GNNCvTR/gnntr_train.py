...
class GNNCvTR(nn.Module):
    def __init__(self, dataset, gnn, support_set, pretrained, baseline):
        ...
        if self.baseline == 0:
            ...
            self.ckp_path_gnn = "checkpoints/checkpoints-baselines/GCN/checkpoint_GIN_gnn_tox21_5.pt"
        ...
        for k in range(0, self.k_train):
            ...
            for t in range(self.train_tasks):
                ...
                for batch_idx, batch in enumerate(tqdm(support_sets[t], desc="Iteration")):
                    ...
                    if self.baseline == 0:
                        pred = self.transformer(self.gnn.pool(node_emb, batch.batch))
                ...
                for batch_idx, batch in enumerate(tqdm(query_sets[t], desc="Iteration")):
                    ...
                    if self.baseline == 0:
                        logit = self.transformer(self.gnn.pool(node_emb, batch.batch))
            ...
            for k in range(0, self.k_test):
                ...
                for batch_idx, batch in enumerate(tqdm(support_set, desc="Iteration")):
                    ...
                    if self.baseline == 0:
                        ...
                        with torch.no_grad():
                            val_logit = self.transformer(self.gnn.pool(node_emb, batch.batch))
